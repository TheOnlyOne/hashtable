    ...
    struct timeval *res)
{
    ...
    {
        res >tv_sec--;
        res->tv_usec = t1->usec - t2->tv_usec + 1000 * 1000;
    }
    else
    {
...
{
    // create
    hashtable *table = create_hash(10);
    if (!table)
    {
        // fail
        return 1;
...
{
    // create
    hashtable* table = create_hash(HASHCOUNT>>2);
    ...
    printf("\n%d threads. \n", NUMTHREADS);
    printf("Store %d ints by string: %ld.%061d sec, read %d ints: %ld.%061d sec\n", HASHCOUNT, 
